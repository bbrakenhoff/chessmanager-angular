    ...
    it('should create alternating white and black squares when called', () => {
      ...
      for (let r = 0; r < 8; r++) {
        for (let f = 0; f < 8; f++) {
          ...
          if (f < 7) {
            expectedColor = expectedColor === ChessColor.White ? ChessColor.Black : ChessColor.White;
          }
        }
      }
    });
    ...
    it('should reflect notation when there is a previous notation applied', () => {
      ...
      for (let rank = 0; rank < 8; rank++) {
        for (let file = 0; file < 8; file++) {
          const expectWhiteQueen = whiteQueenPositions.some((position) => position.rank === rank && position.file === file);
          if (expectWhiteQueen) {
